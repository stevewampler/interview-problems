...
/**
  * From Coursera's Functional Programming Principles in Scala course:
...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def go(
      ...
      if (occurrences.isEmpty)
        ...
        (0 to count).flatMap { count =>
          if (count < 1)
            go(occurrences.tail, combo)
          else
            go(occurrences.tail, (char, count) :: combo)
        }.toList
      }

    go(occurrences).reverse
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.foldLeft(x.toMap) {
      case (xMap, (yChar, yCount)) => xMap.get(yChar).map { xCount =>
        val newCount = xCount - yCount

        if (newCount > 0) {
          xMap.updated(yChar, newCount)
        } else {
          xMap - yChar
        }
      }.getOrElse(xMap)
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def go(sentenceOccurrences: Occurrences, sentence: Sentence = List()): List[Sentence] =
      if (sentenceOccurrences.isEmpty)
        List(sentence)
      else
        combinations(sentenceOccurrences).map { occurrence =>
          occurrence -> dictionaryByOccurrences(occurrence)
        }.filter {
          case (_, words) => words.nonEmpty
        }.flatMap {
          case (occurrence, words) => words.flatMap { word =>
            go(subtract(sentenceOccurrences, occurrence), word :: sentence)
          }
        }
