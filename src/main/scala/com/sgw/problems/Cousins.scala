...
object Cousins {
  ...
  case class Node(name: String, parent: Option[Node] = None)
  ...
  private def nodeToList(nOpt: Option[Node], list: List[Node] = Nil): List[Node] =
    nOpt.map(n => n :: nodeToList(n.parent, list)).getOrElse(list)
  ...
  private def hopsToLCA(n2Opt: Option[Node], hop2: Int, hopMap: Map[Node, Int]): Option[(Int, Int)] =
    n2Opt.flatMap(n2 => hopMap.get(n2).map(hop1 => (hop1, hop2)).orElse(hopsToLCA(n2.parent, hop2 + 1, hopMap)))
