...
/**
  ...
  *
  * From https://www.hackerrank.com/challenges/counting-valleys/problem?h_l=interview&playlist_slugs%5B%5D=interview-preparation-kit&playlist_slugs%5B%5D=warmup&h_r=next-challenge&h_v=zen
  */
object Valleys {
  ...
  def countingValleys(n: Int, s: String): Int = {
    s.map {
      case 'U' => 1
      case 'D' => -1
    }.foldLeft((0, 0)) { case ((oldElev, oldValleys), step) =>
      val newElev = oldElev + step
      val newValleys = if (newElev < 0 && oldElev == 0) {
        oldValleys + 1
      } else {
        oldValleys
      }
